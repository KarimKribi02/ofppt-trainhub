...
const GestionCDC = () => {
  // État pour la gestion des données CDC
  ...
  // Fonction pour récupérer les CDCs depuis l'API
  const fetchCDCs = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/cdcsUsers');
      setCdcs(response.data);
      setError(null);
    } catch (err) {
      setError('Erreur lors du chargement des CDCs. Veuillez réessayer.');
      console.error('Erreur de récupération des CDCs:', err);
    } finally {
      setLoading(false);
    }
  };
  
  // Charger les CDCs au montage du composant
  useEffect(() => {
    fetchCDCs();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://127.0.0.1:8000/api/cdcsUsers', formData);
      ...
      setShowForm(false);
      // Rafraîchir la liste des CDCs
      fetchCDCs();

      // Afficher une notification de succès
      alert('CDC ajouté avec succès !');
    } catch (err) {
      setError('Erreur lors de l\'ajout du CDC. Veuillez réessayer.');
      console.error('Erreur d\'ajout du CDC:', err);
    }
  };

  // Gérer la suppression d'un CDC
  const handleDelete = async (id) => {
    if (window.confirm('Voulez-vous vraiment supprimer ce CDC ?')) {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/cdcsUsers/${id}`);
        fetchCDCs(); // rafraîchir la liste
      } catch (err) {
        console.error('Erreur lors de la suppression du CDC:', err);
        setError('Erreur lors de la suppression du CDC.');
      }
    }
  };
        ...
        <div className="mb-8 md:flex md:items-center md:justify-between">
          ...
          >
            ...
            >
              ...
              {showForm ? 'Annuler' : 'Nouveau CDC'}
            </motion.button>
          </motion.div>
        </div>
        
        {/* Formulaire pour ajouter un nouveau CDC */}
        {showForm && (
          <motion.div 
                        ...
                        {cdcs.data.map((cdc, index) => (
                          ...
                          >
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-gray-900">{cdc.id}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-500 truncate max-w-xs">{cdc.email}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                          <p className="text-sm text-orange-700">
                            Affichage de <span className="font-medium">1</span> à <span className="font-medium">{cdcs.data.length}</span> sur <span className="font-medium">{cdcs.data.length}</span> résultats
                          </p>
                        </div>
                        <div>
