...
const GestionANIMATEUR = () => {
  // État pour la gestion des données ANIMATEUR
  ...
  // Fonction pour récupérer les animateurs depuis l'API
  const fetchanimateurs = async () => {
    ...
    try {
      ...
      setError('Erreur lors du chargement des animateurs. Veuillez réessayer.');
      console.error('Erreur de récupération des animateurs:', err);
    } finally {
      setLoading(false);
    }
  };
  
  // Charger les animateurs au montage du composant
  useEffect(() => {
    fetchanimateurs();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      // Rafraîchir la liste des animateurs
      fetchanimateurs();

      // Afficher une notification de succès
      alert('ANIMATEUR ajouté avec succès !');
    } catch (err) {
      setError('Erreur lors de l\'ajout du ANIMATEUR. Veuillez réessayer.');
      console.error('Erreur d\'ajout du ANIMATEUR:', err);
    }
  };

  // Gérer la suppression d'un ANIMATEUR
  const handleDelete = async (id) => {
    if (window.confirm('Voulez-vous vraiment supprimer ce ANIMATEUR ?')) {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/animateurs/${id}`);
        fetchanimateurs(); // rafraîchir la liste
      } catch (err) {
        console.error('Erreur lors de la suppression du ANIMATEUR:', err);
        setError('Erreur lors de la suppression du ANIMATEUR.');
      }
    }
  };
        ...
        <div className="mb-8 md:flex md:items-center md:justify-between">
          ...
          >
            ...
            >
              ...
              {showForm ? 'Annuler' : 'Nouveau ANIMATEUR'}
            </motion.button>
          </motion.div>
        </div>
        
        {/* Formulaire pour ajouter un nouveau ANIMATEUR */}
        {showForm && (
          <motion.div 
          ...
          <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
              <div className="shadow-lg rounded-lg overflow-hidden border border-orange-100">
                ...
                ) : !animateurs.data || animateurs.data.length === 0 ? (
                  <div className="bg-white px-4 py-12 text-center">
                      ...
                      <tbody className="bg-white divide-y divide-orange-100">
                            {animateurs.map((ANIMATEUR, index) => {
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm text-gray-500 truncate max-w-xs">{ANIMATEUR.email}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                        ANIMATEUR.role === 'admin' 
                                        ? 'bg-purple-100 text-purple-800' 
                                        : ANIMATEUR.role === 'editor' 
                      ...
                      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                          <p className="text-sm text-orange-700">
                            Affichage de <span className="font-medium">1</span> à <span className="font-medium">{animateurs.data.length}</span> sur <span className="font-medium">{animateurs.data.length}</span> résultats
                          </p>
                        </div>
                        <div>
